...
export class ListsService {
    ...
    try {
      ...
      return {
        ...
        films: list.films.map(
          (film) =>
            new ShortInfoFilmEntity({ ...film, avgRating: calculateAvgRating(film.ratings) }),
        ),
      };
    } catch (err) {
  ...
  async changeVisibility(authorId: number, id: number, isPrivate: boolean): Promise<ListEntity> {
    try {
      const list = await this.prisma.listFilms.findUnique({ where: { id } });

      if (!list) {
        throw new HttpException('List not found', HttpStatus.NOT_FOUND);
      }

      if (list.authorId !== authorId) {
        throw new HttpException('You have not access to this list', HttpStatus.FORBIDDEN);
      }
      ...
      if (err instanceof HttpException && (err.getStatus() === 404 || err.getStatus() === 403)) {
        throw err;
      }
      throw new HttpException('Something went wrong', HttpStatus.INTERNAL_SERVER_ERROR);
    }
